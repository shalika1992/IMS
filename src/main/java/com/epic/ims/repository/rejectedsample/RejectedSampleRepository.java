...
public class RejectedSampleRepository {
    ...
    @LogRepository
    @Transactional(readOnly = true)
    public List<RejectedSampleData> getRejectedSampleSearchResultListForReport(RejectedSampleDataInputBean rejectedSampleDataInputBean) {
        List<RejectedSampleData> rejectedSampleDataList = null;
        try {
            StringBuilder dynamicClause = this.setDynamicClause(rejectedSampleDataInputBean, new StringBuilder());
            //create sorting order
            String sortingStr = " order by rd.createdtime desc ";
